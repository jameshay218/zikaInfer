...
posterior <- function(ts, y0s, pars, dat, threshold=NULL){
    ...
    y1 <- ode(y0s,ts1,func="derivs",parms=pars1,dllname="zikaProj",initfunc="initmod",maxsteps=100000,hmax=1e-3,nout=0)
    y0s2 <- y1[nrow(y1),2:ncol(y1)]
    
    y0s2[11] <- I0
    y0s2[5] <- y0s2[5] - I0
    
    y <- ode(y0s2,ts2,func="derivs",parms=pars1,dllname="zikaProj",initfunc="initmod",maxsteps=100000,hmax=1e-3,nout=0)
