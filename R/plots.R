...
plot_setup_data <- function(chain, dat=NULL, incDat=NULL, parTab, ts, local, runs=NULL,
    ...
    incBounds <- as.data.frame(t(sapply(unique(allInc$x),function(x) quantile(allInc[allInc$x==x,"inc"],c(0.025,0.5,0.975)))[c(1,3),]))
    microBounds <- as.data.frame(t(sapply(unique(allMicro$x),function(x) quantile(allMicro[allMicro$x==x,"microCeph"],c(0.025,0.5,0.975)))[c(1,3),]))
    ...
    incBounds$x <- bestInc$x
    microBounds$x <- bestMicro$x
