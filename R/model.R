...
#' Plot best pars
#'
#' Given an MCMC chain of fitting to the multiple data sets, plots the epidemic dynamics
...
#' @export
#' @useDynLib zikaProj
plot_best_pars <- function(chain, dat, parTab, t_pars, local, number, runs=NULL){
    ...
    if(!is.null(runs)){
        ...
        r0s <- numeric(runs)
        for(i in samples){
            bestPars <- as.numeric(chain[i,2:(ncol(chain)-1)])
            
            names(bestPars) <- colnames(chain)[2:(ncol(chain)-1)]
...
#            print(y[which.max(y[,"I_H"]),"times"])
            ...
            pMH <- bestPars["p_MH"]
            tstep <- bestPars["tstep"]
            bp <- bestPars["baselineProb"]
            scale <- bestPars["scale"]
            
            gammaMean <- bestPars["mean"]
            gammaVar <- bestPars["var"]
            ...
            probs[probs > 1] <- 1
            probsDat <- data.frame(weeks=seq(0,39,by=1),prob=probs)
            ...
            allInc <- rbind(allInc, y[,c("times","I_H")])
            predicted <- data.frame(number=predicted, day=tmpDat[,"meanDay"], samp=index)
            index <- index + 1
            allDat <- rbind(allDat, predicted)
            allProbs <- rbind(allProbs, probsDat)
        }
        return(list("predictions"=allDat,"dat"=tmpDat,"incidence"=allInc,"probs"=allProbs,"r0s"=r0s))
    } else {
        ...
        y[,"I_H"] <- y[,"I_H"]/rowSums(y[,c("I_H","E_H","S_H","R_H")])
        y <- as.data.frame(y)
        predicted <- data.frame(number=predicted, day=tmpDat[,"meanDay"])

        
        plot <- ggplot() + geom_point(dat=tmpDat,aes(x=meanDay, y=microCeph)) + geom_line(dat=predicted, aes(x=day,y=number))# + geom_line(dat=y,aes(x=times,y=I_H),col="red")
                                        #plot <- ggplot() + geom_point(dat=as.data.frame(y),aes(x=times,y=I_H))
    }
    return(list(plot,probs)) 
