...
solveSEIRModel_lsoda <- function(ts, y0s, pars,makenames=FALSE){
    ...
    y <- deSolve::ode(y0s, ts, func="C_SEIR_model_lsoda",parms=pars,dllname="zikaProj",initfunc="CinitmodSEIR",nout=0, rtol=1e-5,atol=1e-5)
...
solveSEIRModel_rlsoda <- function(ts, y0s, pars,compatible=FALSE){
    pars <- pars[c("L_M","L_H","D_EM","D_EH","D_IH","b","p_HM","p_MH","constSeed")]
    rlsoda::rlsoda(y0s, ts, C_SEIR_model_rlsoda, parms=pars, dllname="zikaProj", deSolve_compatible = compatible,return_time=TRUE,return_initial=TRUE,atol=1e-6,rtol=1e-6)
}
