...
peakTimes <- data.frame(local=c("bahia","pernambuco","riograndedonorte"),start=c(855,804,862)-60, end = c(855,804,862)+60)
maxChain <- 5
versions <- c(1,2)
priors <- NULL
extra_unfixed <- NULL
...
combos <- expand.grid(runName = "bahia_peak", chainNo=1:maxChain,version=versions,stringsAsFactors=FALSE)
jobs_bahia <- queuer::enqueue_bulk(obj1, combos, "model_fitting_script",
                                  stateNames="bahia",microDat=microDat,incDat=incDat,preParTab=NULL,
                                  allowablePars=NULL,incStates=NULL,
                                  mcmcPars=mcmcPars,mcmcPars2=mcmcPars2,allPriors=priors,
