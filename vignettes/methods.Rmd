...
### Transmission model
The first component of the model captured the transmission dynamics of ZIKV via the *Aedes aegypti* mosquito vector. The model was based on the Ross-MacDonald model for vector-borne disease, capturing deterministic SEIR dynamics in humans with transmission via the mosquito vector experiencing SEI dynamics.[@MacDonald1957] The rationale behind including a transmission model to approximate per capita infection risk is 1) it enables the estimation of the degree of underreporting and epidemiological parameters of interest by fitting the curve to available ZIKV incidence data. In particular, it is possible to estimate the epidemic seed time ($t0$) and the basic reproductive number ($R_0$). The model is defined by the following set of ODEs:
...
```
a [@BrazilPopn]; b [@ColombiaPopn]; c [@BahiaLifeExpectancy]; d[@ColombiaLifeExpectancy].
