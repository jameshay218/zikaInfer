...
# Model description
We developed a two-component model to describe the relationship between the incidence of ZIKV infection and the incidence of microcephaly-affected births, as depicted in Figure S1. Our aim was to estimate the shape and size of the “window of risk” for developing ZIKV-associated microcephaly given infection during gestation, and to test for differences in incidence and risk as predicted by different data sets from various Brazilian states and Colombia.
...
2. By fixing some of the model parameters based on the literature (Table S4), we can infer values for the basic reproductive number, $R_0$, and the epidemic seed time, $t_0$. 
...
## Combined model
Based on the transmission model and microcephaly risk model, the expected proportion of microcephaly affected births (Figure S1E) was calculated by multiplying these two components together. The probability of ZIKV-associated microcephaly affected birth at time, $t$, was therefore given by:
...
# Data
*Table S3. Summary of datasets and sources included in the analysis*
...
Where $t_{peak}$ is the model generated ZIKV peak incidence time; $a$ is the peak time of the ZIKV epidemic in that location based on the day of maximum reported ZIVK incidence; and $b$ is the width of the uniform window around this peak time, representing uncertainty in the timing of the peak. Here, we chose $b$ to be 60 days to represent a ~2 month window of uncertainty around the timing of the ZIKV epidemic peak.
